...
void print_el()
{
	...
	//for the global 2-d list, grab a 1-d row
	for (el = g_lst->next; el != g_lst; el = el->next)
	{

		assert(el);
		//grabs head for the for loop
		es_hd = el->es_head;
		assert(es_hd);

		printf("\n[es] ***** Dumping next event set *****\n");

		//printing out a 1-d row of events
...
void print_event(struct es *es)
{
	...
	//<node number> <port> connected to <node number> on <port>
